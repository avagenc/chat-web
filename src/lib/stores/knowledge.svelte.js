/* Knowledge graph: memori semantik user dari backend — GET /knowledge
   mengembalikan graf UTUH `{nodes, edges}` (backend yang menguras halaman
   Zep; pagination bukan urusan FE). Dimuat lazy saat modal Knowledge Graph
   dibuka, bukan saat login — graf bisa besar dan jarang dilihat. 404 = user
   belum punya graf (belum pernah ngobrol / baru di-reset): bukan error,
   tampil sebagai kosong. */
...
export const knowledgeStore = {
	...
	async load() {
		...
		try {
			...
			const graph = await api('/knowledge');
			const allNodes = graph?.nodes ?? [];
			const allEdges = graph?.edges ?? [];
			/* Paritas explorer Zep: graf utuh selalu tersambung — node tanpa satu
			   edge pun adalah artefak data, bukan pengetahuan, dan cuma jadi debu
			   yatim di kanvas. Saring, kecuali graf memang tak punya edge sama
			   sekali (tampilkan apa adanya daripada kanvas kosong menyesatkan). */
			if (allEdges.length) {
				const linked = new SvelteSet();
				for (const e of allEdges) {
					linked.add(e.source_node_uuid);
					linked.add(e.target_node_uuid);
				}
				nodes = allNodes.filter((n) => linked.has(n.uuid));
			} else {
				nodes = allNodes;
			}
			edges = allEdges;
